...
str_type1='2_Probabilities.h5';
str_type2='0_Probabilities.h5';
...
membrane_matfile = 'MembraneEcad_quantification_BetaCatMembrane.mat';%MembraneEcad_quantification
close all
...
figure,imshowpair(seprate_mask2_clean(k).dat,binary_mask1(k).dat); hold on
...
save('celltype_centroids_masks_wrtCelltype2','nuc_dat1','nuc_dat2','prob_thresh','seprate_mask2_clean','binary_mask1','membrane_masks')
...
nuc_data_matfle = 'celltype_centroids_masks_wrtCelltype2.mat';
load(nuc_data_matfle);
membrane_matfile = 'MembraneEcad_quantification_BetaCatMembrane.mat';%MembraneEcad_quantification
load(membrane_matfile)
nbrh_pxl_sz = 60;%50
% nuc_dat1 = cfp (rfp)  figure(2),plot(celltype2(cellID,1),celltype2(cellID,2),'pb','MarkerFaceColor','b'); hold on
...
%save('celltype_centroids_masks_wrtCelltype2','local2','nbrh_pxl_sz','-append');% rename the
...
save('celltype_centroids_masks_wrtCelltype2','seprate_membrane_intensity_all','seprate_membrane_all','-append');% 
...
[membrane_interface_all,membrane_assign_movie]=...
 ...
 save('celltype_centroids_masks_wrtCelltype2','membrane_interface_all','scale_nbrh','-append');
%%
close all
...
membramne_data_file = 'celltype_centroids_masks_wrtCelltype2.mat';%celltype_centroids_masks
...
ylabel('Freguency');
ylim([0 0.35])
xlim([0 max(like_interface(ii).pxlintentsity)])
h = figure(ii);
h.CurrentAxes.LineWidth = 1.5;
h.CurrentAxes.FontSize = 12;
box on
figure(ii),title('Cells sorting on micropattern');
...
title('Cells sorting on micropattern');
h7 = figure(7);
h7.CurrentAxes.XTick = size(time_point,2);
h7.CurrentAxes.XTickLabel = time_point;
end
 ylim([0 3000]);
 xlim([0 6]);
 ylabel('BetaCat-Ecad in section of the membrane interface');
 legend('Betacat-Ecad on like interfaces','Betacat-Ecad on unlike interfaces');
